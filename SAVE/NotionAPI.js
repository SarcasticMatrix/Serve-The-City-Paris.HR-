...
async function readDatabaseEvent(databaseId) {
  // Return en json la liste des pages des personnes acceptées, non-cochées

  const database = await notion.databases.query({database_id: databaseId, filter: { and: [{"property":"Accept ?","select":{equals:"Accepted"}}]}});
  const listPages = database.results;
  if (listPages.length==0) {
      console.log("There is no accepted member.");
  }
  else {
      console.log(`Event mail sent to ${listPages.length} SAVE members.`);
  }
  //const listProperties = database.results[0].properties; // await notion.pages.retrieve( { page_id: listPages.results[0].id });
  return listPages;
}
...
async function function_listEvent(databaseId){
  // return la liste des pageId des personnes acceptées et refusées
  const listPages = await readDatabaseEvent(databaseId);
  //console.log(listPages);
...
module.exports= {
    ...
    readDatabaseEvent:readDatabaseEvent,
    readProperty:readProperty,
    function_list:function_list,
    function_listEvent:function_listEvent,
    readPage:readPage,
    checkMail:checkMail,
